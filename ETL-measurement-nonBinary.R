...
listcolnames_Measurement_nonBinary
missing_cols <- setdiff(c("SUBJID", listcolnames_Measurement_nonBinary), colnames(input_data))
missing_cols

vertical_table_Measurement_nonBinary <- data.frame()
if (length(missing_cols) == 0) {
  
  # Select specific columns from input_data based on listcolnames_Measurement_nonBinary
  selected_columns <- input_data[, c("SUBJID", listcolnames_Measurement_nonBinary), drop = FALSE]
...
#vertical_table_Measurement_nonBinary$Value <- as.numeric(vertical_table_Measurement_nonBinary$Value)
#vertical_table_Measurement_nonBinary$unit_concept_id <- as.numeric(vertical_table_Measurement_nonBinary$unit_concept_id)

vertical_table_Measurement_nonBinary
...
vertical_table_Measurement_nonBinary <- na.omit(vertical_table_Measurement_nonBinary, cols = c("vertical_table_Measurement_nonBinary"))
...
for (i in 1:nrow(vertical_table_Measurement_nonBinary)) {
  ...
  person_id <- get_person_id(con, vertical_table_Measurement_nonBinary$SUBJID[i])
  ...
  value_source_value <- gsub(",", ".", vertical_table_Measurement_nonBinary$Value[i], fixed = TRUE)
