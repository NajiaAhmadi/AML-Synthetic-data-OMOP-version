...
#dbExecute(con, "TRUNCATE TABLE cds_cdm_01.condition_occurrence CASCADE;")
...
missing_cols <- setdiff(c("SUBJID", listcolnames), colnames(input_data))

vertical_table <- data.frame()
if (length(missing_cols) == 0) {
  
  # Select specific columns from input_data based on listcolnames
  selected_columns <- input_data[, c("SUBJID", listcolnames), drop = FALSE]
...
for (i in 1:nrow(vertical_table)) {
  ...
  person_id <- get_person_id(con, vertical_table$SUBJID[i])
  ...
  query <- glue::glue("
    INSERT INTO cds_cdm_01.condition_occurrence (
...
dbDisconnect(con)
cat("Measurement data inserted into cds_cdm_01.condition_occurrence table.\n")
