...
listcolnames_observation_nonBinary

missing_cols <- setdiff(c("SUBJID", listcolnames_observation_nonBinary), colnames(input_data))
missing_cols

if (length(missing_cols) == 0) {
  
  # Select specific columns from input_data based on listcolnames_observation_nonBinary
  selected_columns <- input_data[, c("SUBJID", listcolnames_observation_nonBinary), drop = FALSE]
...
vertical_table_observation_nonBinary$unit_concept_id <- as.integer(vertical_table_observation_nonBinary$unit_concept_id)
...
#for (i in seq(nrow(vertical_table_observation_nonBinary))) {
#  current_unit_concept_id <- vertical_table_observation_nonBinary$unit_concept_id[i]
#  cleaned_unit_concept_id <- gsub("[^0-9]", "", current_unit_concept_id)
#  vertical_table_observation_nonBinary$Concept_id[i] <- ifelse(nchar(cleaned_unit_concept_id) > 0, cleaned_unit_concept_id, NA)
#}
...
for (i in 1:nrow(vertical_table_observation_nonBinary)) {
  ...
  person_id <- get_person_id(con, vertical_table_observation_nonBinary$SUBJID[i])
  observation_concept_id <- vertical_table_observation_nonBinary$Concept_id[i]
  observation_date <- "01.01.1800" # Ensure this date format is accepted by your database
  observation_type_concept_id <- "32817"
  observation_source_value <- vertical_table_observation_nonBinary$Variable[i]
  
  # Format value_as_number to ensure it uses a period as the decimal separator
  value_as_number <- gsub(",", ".", vertical_table_observation_nonBinary$Value[i], fixed = TRUE)
  ...
  ")
  print(as.character(query)) # Debugging: Check the query before execution
